...
/**
 * HttpLogInterceptor
 * Created by Khorium on 2016/9/20.
 */
class HttpLogInterceptor implements Interceptor {
    ...
    HttpLogInterceptor(HttpLogInterceptor.Logger logger) {
        this.logger = logger;
    }
    ...
    HttpLogInterceptor setLevel(HttpLogInterceptor.Level level) {
...
//        logger.log(requestStartMessage);
        message.append(requestStartMessage);
...
//                logger.log("--> END " + request.method() + " (encoded body omitted)");
                message.append("\n").append("--> END ").append(request.method()).append(" (encoded body omitted)");
            } else {
...
//                + bodySize + " body" : "") + ')');
        message.append("\n").append("<-- ").append(response.code()).append(' ').append(response.message()).append(' ').append(response.request().url()).append(" (").append(tookMs).append("ms").append(!logHeaders ? ", "
...
//                        logger.log("<-- END HTTP");
                        message.append("\n").append("").append("\n").append("Couldn't decode the response body; charset is likely malformed.").append("\n").append("<-- END HTTP");
                        return response;
                    }
                }
...
//                logger.log("<-- END HTTP (" + buffer.size() + "-byte body)");
    ...
    private static String formatJson(String json) {
        ...
        try {
            ...
            return "Invalid Json, origin:"+json;
